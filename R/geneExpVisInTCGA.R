#' Plot boxplots for multiple genes in a single cancer type
#'
#' This function generates boxplots to visualize the expression of multiple genes in a single cancer type,
#' with statistical comparisons between two groups. It automatically handles extreme outliers using quantiles.
#'
#' @param boxdata A data frame containing gene expression data with required columns:
#'   \code{gene} (gene names), \code{exp} (expression values), \code{group} (comparison groups),
#'   and \code{cancer} (cancer type)
#' @param color A character vector of length 2 specifying colors for the two comparison groups.
#'   Defaults to c("#E7B800", "#2E9FDF")
#' @param facet A logical value indicating whether to use facet_wrap by cancer type.
#'   Automatically set to FALSE if only one cancer type is present. Defaults to TRUE
#'
...
#' Plot boxplots for a single gene across multiple cancer types
#'
#' This function generates boxplots to visualize the expression of a single gene across multiple cancer types,
#' with statistical comparisons between two groups in each cancer type.
#'
#' @param boxdata A data frame containing gene expression data with required columns:
#'   \code{gene} (gene name), \code{exp} (expression values), \code{group} (comparison groups),
#'   and \code{cancer} (cancer types)
#' @param color A character vector of length 2 specifying colors for the two comparison groups.
#'   Defaults to c("#E7B800", "#2E9FDF")
#'
#' @return A ggplot2 object
...
#' Generate boxplot and violin plot for a single gene in one cancer type
#'
#' This function creates both boxplot and violin plot (with embedded boxplot) to visualize
#' the expression of a specific gene in a specific cancer type, including statistical testing.
#'
#' @param data A data frame containing gene expression data with required columns:
#'   \code{gene} (gene names), \code{exp} (expression values), \code{Sample} (sample groups),
#'   \code{cancer} (cancer types), and \code{id} (sample identifiers)
#' @param cancer A character string specifying the cancer type to plot
#' @param gene A character string specifying the gene to plot
#' @param color A character vector of length 2 specifying colors for the two sample groups.
#'   Defaults to c("#E7B800", "#2E9FDF")
#'
#' @return A list containing two ggplot2 objects: \code{violin} (violin plot) and \code{boxplot} (boxplot)
...
#' Plot pan-cancer expression for a single gene with statistical testing
#'
#' This function generates a pan-cancer boxplot with scatter points to visualize the expression
#' of a single gene across multiple cancer types, with optional paired statistical testing.
#'
#' @param data A data frame containing gene expression data with required columns:
#'   \code{gene} (gene names), \code{exp} (expression values), \code{group} (comparison groups),
#'   \code{cancer} (cancer types), and \code{Sample} (sample groups)
#' @param gene A character string specifying the gene to plot
#' @param paired A logical value indicating whether to perform paired statistical testing
#'   (adjusts group mapping and test parameters)
#' @param color A character vector of length 2 specifying colors for the two groups.
#'   Defaults to c("#E7B800", "#2E9FDF")
#'
#' @return A ggplot2 object
...
#' Generate paired point-line plot for a single gene in one cancer type
#'
#' This function creates a point-line plot to visualize paired expression data of a specific gene
#' in a specific cancer type (typically comparing normal and tumor samples from the same patients),
#' including paired t-test results.
#'
#' @param data A data frame containing paired gene expression data with required columns:
#'   \code{gene} (gene names), \code{exp} (expression values), \code{Sample} (sample groups,
#'   must include "Normal" and "Tumor"), \code{cancer} (cancer type), and \code{id} (patient identifiers)
#' @param cancer A character string specifying the cancer type to plot
#' @param gene A character string specifying the gene to plot
#' @param color A character vector of length 2 specifying colors for the two sample groups.
#'   Defaults to c("#E7B800", "#2E9FDF")
#' @param linecolor A character string specifying the color for connecting lines.
#'   Defaults to "#1A1A1A"
#'
#' @return A ggplot2 object
